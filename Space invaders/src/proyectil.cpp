...
void Disparar(Proyectil** cabeza, Vector2 pos, float velY) {
    ...
    nuevo->posicion = pos;
    nuevo->velocidadY = velY; // Si es -7 sube, si es +7 baja
    nuevo->ancho = 4.0f;
    nuevo->alto = 12.0f;
...
void ActualizarProyectiles(Proyectil** cabeza, int altoPantalla) {
    ...
    while (actual != NULL) {
        actual->posicion.y += actual->velocidadY; // Sumamos la velocidad con su signo

        // Eliminamos si sale por arriba (Y < 0) O por abajo (Y > altoPantalla)
...
void LiberarProyectiles(Proyectil** cabeza) {
    ...
    *cabeza = NULL; // Dejamos la lista completamente vacía
}